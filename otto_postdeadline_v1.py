...
from rf_module import do_RF_v1
...
def do_gbdt(train_x, train_y, learning_rate=0.01, max_depth=5, max_features='auto', n_estimators=6000,
    ...
    if os.path.exists(mdl_name)==True and load==True:
...
clf_rf = do_RF_v1(train_x, train_y)

#clf_rf = RandomForestClassifier(n_estimators=700, max_depth=10)
print_clf(clf_rf, test_x, test_y, clf_name="RF calibratedCV", outfname=outfile)
clfs.append(clf_rf)
...
#           ('output', DenseLayer)]
#
#clf_nn3 = NeuralNet(layers=layers0,
...
#                on_epoch_finished=[AdjustVariable('update_learning_rate', start=learn_r)],
#                eval_size=0.05,
#                verbose=1,
#                max_epochs=17)

#clf_nn3.fit(train_x2, train_y2)
